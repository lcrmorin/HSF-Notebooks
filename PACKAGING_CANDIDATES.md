...
| Function | Defined in | What it does | Why it's a candidate | Status |
|---|---|---|---|---|
| `wiechel_forward(lon_deg, lat_deg, R=1.0)` | `12b_geodesy_wind_and_map_projections.ipynb` | Closed-form polar-aspect Wiechel projection: $x=R(\sin\lambda\cos\varphi-(1-\sin\varphi)\cos\lambda)$, $y=-R(\cos\lambda\cos\varphi+(1-\sin\varphi)\sin\lambda)$. Neither PROJ (confirmed absent from pyproj's full operations map) nor `Basemap` implement Wiechel, so this is a from-scratch port, sourced from a HandWiki mirror of the Wikipedia article (the primary page's formulas render as un-scraped images) and verified via the algebraic identity $x^2+y^2=2R^2(1-\sin\varphi)$. | Single-use so far (`coordinate6`), but this chapter is full of MATLAB Mapping-Toolbox `axesm` calls, and Wiechel is one of several named projections the book cycles through for map-based hazard visuals later in Chapter 12 — worth reusing directly if another script asks for it, rather than re-deriving. | 🔵 |
| GSHHS land-polygon extraction via a `'cyl'`-projection `Basemap` (`Basemap(projection="cyl",...).landpolygons[i].boundary` recovers raw unprojected (lon, lat) vertices, since a plate-carree projection's x/y equal lon/lat directly) | `12b_geodesy_wind_and_map_projections.ipynb` | Works around `cartopy`'s Natural Earth download being blocked by this sandbox's network egress: `basemap-data`'s bundled GSHHS shapefiles give offline land geometry, and this trick recovers it in plain lon/lat for use in projections `Basemap` itself doesn't support (like Wiechel), rather than being locked into Basemap's own supported projection list. | The general offline-land-data pattern for any future map in this series that needs raw coastline geometry rather than a `Basemap`-native projection — resolves the `cartopy`-network-dependency risk flagged in the original Chapter 12 scoping report for good. Single-use so far. | 🔵 |

## Chapter 12c

| Function | Defined in | What it does | Why it's a candidate | Status |
|---|---|---|---|---|
| `compute_horizontal_resolution(delta_phi, delta_lambda)` | `12c_ipcc_grid_resolution_formulas.ipynb`, ported from the archive's own `compute_horizontal_resolution.m` (chapter-12 root, not a numbered exercise) | Returns `(R_0, R_45, R_avg)`: the IPCC-style grid-cell horizontal resolution at the equator and at 45° latitude, plus the `0.7628`-shortcut average, at a hardcoded `length_phi=111.2` km/deg. | The archive itself already factored this out as a shared helper used by `grid2e` and (per the scoping report) `grid3*` — a straightforward, faithful, single-purpose port; flag directly reused rather than re-derived the moment `12d`'s `grid3*` scripts are built. | 🔵 |

## Chapter 12d

| Function | Defined in | What it does | Why it's a candidate | Status |
|---|---|---|---|---|
| `resolution_to_angular(res)` | `12d_ipcc_assessment_report_grids.ipynb`, ported from `Data/chap12_ipcc_grid.m`'s `Resolution_to_Angular`/`Rhomboidal_to_Angular`/`Triangular_to_Angular`/`Format_Angular` | Converts a climate-model grid-resolution label in any of three notations (`"R21"` rhomboidal, `"T63"` triangular spectral truncation, `"2.5°x3.75°"` plain angular) into a `(delta_phi, delta_lambda)` degree pair. | The archive's own grid-metadata table (`chap12_ipcc_grid.xlsx`) mixes all three notations across its FAR-AR6 sheets — any later script that touches this table's `A_Resolution`/`O_Resolution` columns needs the same conversion. Confirmed reused across the 4 sheets analyzed in `grid3a-d` already. | 🔵 |
| `compute_stats(x)` | `12d_ipcc_assessment_report_grids.ipynb` | `[n, median, min, max, mean]` of a 1-D array, treating `n` as the *original* length (including `NaN`s) while the summary stats themselves drop `NaN`s first — matches the source's own `compute_stats.m` semantics (`n=size(x,1)` before `packr`). | A trivial-looking but easy-to-get-subtly-wrong convention (most people would compute `n` after dropping `NaN`s) — worth reusing verbatim rather than re-deriving whenever a later script's `disp(ftosa([n median min max mean],...))` pattern reappears; used 8 times across `12d` alone (once per `grid3a-d` resolution/level block, twice more across `grid3e`/`3f`). | 🟡 |

## Chapter 12i

| Function | Defined in | What it does | Why it's a candidate | Status |
|---|---|---|---|---|
| `pdfweibull`/`cdfweibull`/`cdfweibulli`/`skewness_weibull`, `pdfgev`/`cdfgev`/`cdfgevi` | `12i_extreme_value_distributions.ipynb`, reimplemented from formula (not in the archive) | Weibull(scale, shape) and GEV(location, scale, shape) pdf/cdf/inverse-cdf/skewness in the MATLAB parameter convention, cross-checked against `scipy.stats.weibull_min`/`genextreme` to ~1e-17. | Chapter 13 (Stress Testing, not yet scoped) plausibly reuses extreme-value distributions for tail-risk/stress scenarios — these are provably-correct, ready-made ports rather than something to re-derive from scratch if that need arises. | 🔵 |

## Chapter 12g

| Function | Defined in | What it does | Why it's a candidate | Status |
|---|---|---|---|---|
| `ecb_risk_score_table(hzrd_typ)` / `ecb_loss_table(hzrd_typ)` | `12g_financial_loss_and_catastrophe_metrics.ipynb`, each called twice (`4a`/`4b`, `4c`/`4d`) | Filter the ECB physical-risk stress-test tables to portfolio-level rows for one hazard scenario, sort by country/activity, and select/rename/rescale the relevant score or loss columns by name (sidestepping the `T{:,17:20}`-style MATLAB positional-offset trap documented in the Progress notes). | Confirmed reused twice each within `12g` already; a third ECB hazard scenario elsewhere in the book (the archive's `ext_ecb_physical_risk.xlsx` covers more `hzrd_typ` values than the 4 this chapter's exercises use) would reuse either directly. | 🟡 |
