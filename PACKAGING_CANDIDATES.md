...
| `corr_from_lower_triangle(values, n)` | `03a_esg_risk_and_factors.ipynb` | Builds a symmetric n x n correlation matrix from its lower-triangular entries listed row by row (the MATLAB/GAUSS `xpnd` convention the `.m` source uses everywhere a correlation matrix is hand-specified). | A generic array-construction utility with no ESG content — the `xpnd`-style literal shows up throughout this book's `.m` source (`markowitz_param1`, `te_param1`, `pastor6`, `pedersen1-4d` all hand-specify a correlation matrix this way), so later notebooks are very likely to want it too. | 🔵 |
